...
# Date: 2-11-2024
ecl_char = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z']
def check_cid(passport):
    if 'cid' in passport:
        ...
        if 99 < cid_value < 1000:
            try:
                int(passport[(passport.index('cid')+8):(passport.index('cid')+8)])
            except:
                return True
            else:
                return False
...
def check_pid(passport):
    ...
    if 'pid' in passport:
        ...
        try:
            int(passport[passport.index('pid')+14:passport.index('pid')+14])
        except:
...
def check_ecl(passport):
    ...
    if 'ecl' in passport:
        ...
        try:
            int(passport[passport.index('ecl')+8:passport.index('ecl')+8])
        except:
            if str(passport[passport.index('ecl')+8:passport.index('ecl')+8]) in ecl_char:
                return False
...
def check_hcl(passport):
    ...
    if 'hcl' in passport:
        ...
        if hcl_value[0] == "#":
            #print("pass the # test")
            if str(passport[passport.index('hcl')+12:passport.index('hcl')+12]) in valid_char:
                return False
            for i in range(1,7):
...
def check_hgt(passport):
    if 'hgt' in passport:
        if 'cm' in passport and (150 <= int(passport[passport.index('cm')-3:passport.index('cm')]) <= 193):
            try:
                int(passport[passport.index('cm')-4])
            except:
            ...
            try:
                int(passport[passport.index('in')-3])
            except:
...
def check_eyr(passport):
    if 'eyr' in passport and (2024 <= int(passport[passport.index('eyr')+4:passport.index('eyr')+8]) <= 2034):
        try:
            int(passport[(passport.index('eyr')+9):(passport.index('eyr')+9)])
        except:
            return True
        else:
            return False
...
def check_byr(passport):# take in one passport 
    if 'byr' in passport and (1920 <= int(passport[(passport.index('byr')+4):(passport.index('byr')+8)]) <= 2008):
        try:
            int(passport[(passport.index('byr')+9):(passport.index('byr')+9)])
        except:
            return True
        else:
            return False
...
if user_query == 'scanned_passports.txt':
    print('There are', valid_passports, 'valid passports')
else:
    print('There are', valid_passports - 1, 'valid passports')
