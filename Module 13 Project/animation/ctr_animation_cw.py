...
class ctr_left_cup:
    ...
    def draw(self):
        if (the_ctr_right_cup.return_values()[0] != the_ctr_right_cup.return_values()[2]) or (the_ctr_right_cup.return_values()[1] != the_ctr_right_cup.return_values()[3]):
            SCREEN.blit(self.image,(self.x, self.y))
    def update(self):'''
...
def ctr_animation_cw_reset():
    the_ctr_right_cup.reset()
    the_ctr_mid_cup.reset()
def ctr_animation_cw():
    ...
    This animation switches the right and the center cup in a clockwise motion.'''
